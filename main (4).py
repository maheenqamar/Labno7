...
class FormApp:
  ...
  def __init__(self, master):
    ...
    self.cursor.execute('''CREATE TABLE IF NOT EXISTS users
                             (Name TEXT, Email TEXT, Phone_Number TEXT, Address TEXT, Job_Title TEXT, Income TEXT)'''
                        )
  ...
  def submit_form(self):
    ...
    self.cursor.execute("INSERT INTO users VALUES (?, ?, ?, ?, ?, ?)",
                        (name, email, phone, address, job_title, income))
    self.conn.commit()

    messagebox.showinfo("Success", "Form submitted in database successfully")
    self.master.destroy()
